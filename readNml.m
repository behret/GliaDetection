...
%
    ...
    if ~isempty( fname )
        ...
        output{1}.parameters = get_xml_element_as_struct( content, 'parameters');

        % write section COMMENTS from content into output struct
        output{1}.commentsString = get_xml_element_as_string( content, 'comments');
        ...
        for count_thing = 1 : size( thing, 2 )
            ...
            output{count_thing}.nodes = repmat( 0, [size( krk_theseNodes, 2 ), 5] );
            ...
            output{count_thing}.edges = repmat( 0, [size( krk_theseEdges, 2 ), 2] );
